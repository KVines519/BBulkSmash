...
# Regex patterns defined as constants to avoid tool corruption issues
PHONE_OR_ALPHANUM_RE = r'^(\+[0-9]+|[a-zA-Z0-9]+)' + r'$'
HOSTNAME_OR_IP_RE = (
...
class UACForm(forms.ModelForm):
    ...
    called_party = forms.CharField(
        ...
        max_length=50,
        required=False,
        validators=[RegexValidator(regex=PHONE_OR_ALPHANUM_RE, message='Enter a valid phone number or alphanumeric ID.')]
    )
    ...
    auto_accept_options = forms.BooleanField(
        label='Auto-Accept OPTIONS',
        required=False,
        initial=False
    )
    ...
    stun_server = forms.CharField(
        ...
        max_length=255,
        required=False,
        initial='stun.cloudflare.com',
        help_text='IP address or hostname (e.g. stun.cloudflare.com)',
        validators=[
            RegexValidator(
                regex=HOSTNAME_OR_IP_RE,
                message='Enter a valid IP address or hostname (e.g. stun.cloudflare.com or 1.2.3.4)'
            )
        ]
    )
    ...
    class Meta:
        ...
        fields = [
            ...
            'min_rtp_port', 'max_rtp_port', 'auto_accept_options',
        ]
...
class UASForm(forms.ModelForm):
    ...
    uas_local = forms.GenericIPAddressField(label='UAS Local Address', protocol='IPv4')
...
class xpcUploadForm(forms.Form):
    file = forms.FileField(
        label='Select an XML, PCAP, CSV, or WAV file',
        help_text='XML file name should start with "uac" or "uas".',
        widget=forms.ClearableFileInput(attrs={'accept': '.xml, .pcap, .csv, .wav', 'max_upload_size': 1048576})
    )

    def clean_file(self):
        uploaded_file = self.cleaned_data.get('file')
        if not uploaded_file.name.lower().endswith(('.pcap', '.xml', '.csv', '.wav')):
