...
const pcapList = document.getElementById('pcap_list');
const wavList = document.getElementById('wav_list');
const csvList = document.getElementById('csv_list');
const logList = document.getElementById('log_list');


function createPcapCsvWavListItem(fileName){
    ...
    deleteFile.href = `/file-management/?delete=${fileName}`;
    deleteFile.className = 'xml-li-button';
    deleteFile.title = 'Delete';
    deleteFile.onclick = function() {
        return confirm(`Are you sure you want to delete this file: ${fileName}?`);
    };
    deleteFile.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" height="16px" width="16px" fill="currentColor"><path d="M135.2 17.7L128 32 32 32C14.3 32 0 46.3 0 64S14.3 96 32 96l384 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-96 0-7.2-14.3C307.4 6.8 296.3 0 284.2 0L163.8 0c-12.1 0-23.2 6.8-28.6 17.7zM416 128L32 128 53.2 467c1.6 25.3 22.6 45 47.9 45l245.8 0c25.3 0 46.3-19.7 47.9-45L416 128z"/></svg>`;
...
pcapFiles.forEach(file => {
    const listItem = createPcapCsvWavListItem(file);
    pcapList.appendChild(listItem);
});

wavFiles.forEach(file => {
    const listItem = createPcapCsvWavListItem(file);
    wavList.appendChild(listItem);
});
...
function createLogListItem(fileName) {
    const listItem = document.createElement('li');
    const spanElm = document.createElement('span');
    spanElm.textContent = `${fileName} `;
    ...
    // Download link
    const downloadLink = document.createElement('a');
    downloadLink.href = `/download-log/${fileName}`;
    downloadLink.download = fileName;
    downloadLink.className = 'xml-li-button';
    downloadLink.title = 'Download';
    downloadLink.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" height="16px" width="16px" fill="currentColor"><path d="M288 32c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 242.7-73.4-73.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l128 128c12.5 12.5 32.8 12.5 45.3 0l128-128c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L288 274.7 288 32zM64 352c-35.3 0-64 28.7-64 64l0 32c0 35.3 28.7 64 64 64l384 0c35.3 0 64-28.7 64-64l0-32c0-35.3-28.7-64-64-64l-101.5 0-45.3 45.3c-25 25-65.5 25-90.5 0L165.5 352 64 352zm368 56a24 24 0 1 1 0 48 24 24 0 1 1 0-48z"/></svg>`;
    ...
    deleteFile.title = 'Delete';
    deleteFile.onclick = function() {
        return confirm(`Are you sure you want to delete this log: ${fileName}?`);
    };
    deleteFile.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" height="16px" width="16px" fill="currentColor"><path d="M135.2 17.7L128 32 32 32C14.3 32 0 46.3 0 64S14.3 96 32 96l384 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-96 0-7.2-14.3C307.4 6.8 296.3 0 284.2 0L163.8 0c-12.1 0-23.2 6.8-28.6 17.7zM416 128L32 128 53.2 467c1.6 25.3 22.6 45 47.9 45l245.8 0c25.3 0 46.3-19.7 47.9-45L416 128z"/></svg>`;
...
logFiles.forEach(file => {
    const listItem = createLogListItem(file);
...
function createXmlListItem(xmlName){
    ...
    editLink.href=`/edit-xml/?xml=${xmlName}&back=file-management`;
    ...
    deleteXml.href = `/file-management/?delete=${xmlName}`;
...
setupDropdownToggle('pcap-list-b', 'pcap_list');
setupDropdownToggle('csv-list-b', 'csv_list');
setupDropdownToggle('wav-list-b', 'wav_list');
setupDropdownToggle('log-list-b', 'log_list');
