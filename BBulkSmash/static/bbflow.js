...
// Register modal close handlers once at load time
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (span) {
        span.onclick = () => {
            ...
            modal.style.visibility = 'visible'; // reset for next open
        };
    }
...
// Single click handler for all .show-flow elements — renders diagram and opens modal
document.addEventListener('click', async (event) => {
    if (!event.target || !event.target.classList.contains('show-flow')) return;
    ...
    // Show modal as invisible first so containers have real dimensions for the library
    modal.style.visibility = 'hidden';
    modal.style.display = 'block';

    // Wait one frame for the browser to complete layout before measuring
    ...
    // Now reveal the modal with diagrams fully rendered
