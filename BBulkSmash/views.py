...
from .models import UacAppConfig, UasAppConfig, AppSettings
from .scripts.bbsipp import get_sipp_processes, clean_filename, run_uac, run_uas, delete_old_screen_logs, build_uac_command, build_uas_command, is_sipp_pid
...
def _safe_path(base_dir: Path, filename: str) -> Path:
    """
    Resolve `filename` relative to `base_dir` and verify it stays within
    `base_dir`. Raises ValueError on path traversal attempts.
    Used by all file-serving and file-writing views.
    """
    # Reject obvious traversal characters before even resolving
...
def index(request):
    ...
    if request.method == 'POST':
        ...
        elif 'toggle_trace' in request.POST:
            trace = AppSettings.get()
            flag = request.POST.get('toggle_trace')
            if flag in ('trace_stat', 'trace_msg', 'trace_counts', 'trace_err',
                        'capture_sip', 'capture_sip_rtp'):
                # capture_sip and capture_sip_rtp are mutually exclusive
                if flag == 'capture_sip':
                    new_val = not trace.capture_sip
                    trace.capture_sip = new_val
                    if new_val:
                        trace.capture_sip_rtp = False
                elif flag == 'capture_sip_rtp':
                    new_val = not trace.capture_sip_rtp
                    trace.capture_sip_rtp = new_val
                    if new_val:
                        trace.capture_sip = False
                else:
                    setattr(trace, flag, not getattr(trace, flag))
                trace.save()
            return redirect('index')

        elif 'pre_run_delay' in request.POST:
            trace = AppSettings.get()
            try:
                delay = max(0, min(30, int(request.POST.get('pre_run_delay', 0))))
            except (ValueError, TypeError):
                delay = 0
            trace.pre_run_delay = delay
            trace.save()
            return redirect('index')
            ...
            if send_signal == 'Kill':
                try:
                    ...
                    if not is_sipp_pid(pid_int):
                        logger.warning(f"Kill rejected: PID {pid_int} is not a known SIPp process")
                    else:
                ...
                except (psutil.NoSuchProcess, psutil.TimeoutExpired, ValueError) as e:
                    logger.exception(e)
                    pass
                ...
                # For manually-launched SIPp (no -sf flag), script_name is "None"
                xml_wo_ext = script_name.rsplit(".", 1)[0] if script_name and script_name != 'None' else f"pid{pid}"
                try:
    ...
    trace_settings = AppSettings.get()
    try:
        print_uac_command = build_uac_command(uac_config, trace_settings)
    except Exception:
        print_uac_command = ''
    try:
        print_uas_command = build_uas_command(uas_config, trace_settings)
    except Exception:
        print_uas_command = ''
...
def serve_xml_file(request, xmlname):
    ...
    try:
        xml_path = _safe_path(xml_dir, xmlname)
    except ValueError:
...
def serve_pcap_csv(request, filename):
    xml_base = Path(settings.BASE_DIR) / 'BBulkSmash' / 'xml'
    try:
        if filename.lower().endswith('.pcap'):
            file_path = _safe_path(xml_base / 'pcap', filename)
            content_type = 'application/vnd.tcpdump.pcap'
        elif filename.lower().endswith('.csv'):
            file_path = _safe_path(xml_base / 'csv', filename)
            content_type = 'text/csv'
        elif filename.lower().endswith('.wav'):
            file_path = _safe_path(xml_base / 'wav', filename)
...
def serve_log_file(request, filename):
    ...
    allowed_extensions = ('.log', '.csv', '.pcap')
    if not any(filename.endswith(ext) for ext in allowed_extensions):
        return HttpResponse('Log file not found', status=404)
    try:
        log_path = _safe_path(log_dir, filename)
    except ValueError:
        return HttpResponse('Invalid filename', status=400)
    ...
    if filename.endswith('.pcap'):
        with open(log_path, 'rb') as f:
            content = f.read()
        response = HttpResponse(content, content_type='application/vnd.tcpdump.pcap')
    else:
...
def xml_editor(request):
    ...
    if request.method == 'GET':
        ...
        try:
            full_path = _safe_path(xml_path, xmlName)
        except ValueError:
            return HttpResponse('Invalid filename', status=400)
...
def sngrep_view(request):
    from .models import UacAppConfig, UasAppConfig
    # Collect unique SIP ports from active UAC/UAS configs for the BPF filter
    ports = set()
    uac = UacAppConfig.objects.filter(is_active_config=True).first()
    if uac:
        ports.add(uac.uac_local_port)
        ports.add(uac.uac_remote_port)
    uas = UasAppConfig.objects.filter(is_active_config=True).first()
    if uas:
        ports.add(uas.uas_local_port)
        ports.add(uas.uas_remote_port)
    # Fall back to 5060 if no active config exists
    if not ports:
        ports.add(5060)
    return render(request, 'sngrep.html', {
        'bpf_ports': sorted(ports),
    })
...
def file_mgmt_view(request):
    ...
    context = {
        ...
        'pcap_list': pcap_list, 'csv_list': csv_list,
        'wav_list': wav_list, 'log_list': log_list,
...
def delete_files_view(request):
    """AJAX endpoint: POST JSON list of filenames to delete. Returns JSON result."""
    if request.method != 'POST':
    ...
    try:
        ...
        return HttpResponse(_json.dumps({'error': 'Invalid request'}),
    ...
    for name in filenames:
        # Reject obvious traversal characters before resolving
        ...
        try:
            ...
            if name_lower.endswith('.xml'):
                p = _safe_path(xml_dir, name)
            elif name_lower.endswith('.wav'):
                p = _safe_path(wav_dir, name)
            elif name_lower.endswith('.pcap'):
                # Could be in pcap_dir (audio files) or log_dir (sngrep captures)
                p = _safe_path(pcap_dir, name) if (pcap_dir / name).exists() else _safe_path(log_dir, name)
            elif name_lower.endswith('.csv'):
                # Could be in csv_dir (injection) or log_dir (trace)
                p = _safe_path(csv_dir, name) if (csv_dir / name).exists() else _safe_path(log_dir, name)
            elif name_lower.endswith('.log'):
                p = _safe_path(log_dir, name)
            else:
                errors.append(name)
                continue
            ...
            logger.exception(f"Error deleting {name}: {e}")
            errors.append(name)

    return HttpResponse(_json.dumps({'deleted': deleted, 'errors': errors}),
