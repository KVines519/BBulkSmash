...
ChangedAddressError = "Error encountered on Changed IP and Port"
...
def stun_test(sock, host, port, source_ip, source_port, send_data=""):
    ...
    while not recvCorr:
        ...
        kcount -= 1
        if kcount == 0:
            log.warning("stun_test: max retries reached without a correlated response")
            retVal['Resp'] = False
            break
...
def get_nat_type(s, source_ip, source_port, stun_host=None, stun_port=3478):
    ...
    if ret['ExternalIP'] == source_ip:
        ...
        if ret['Resp']:
            ...
            log.debug("Do Test1")
            if changedIP is not None and changedPort is not None:
